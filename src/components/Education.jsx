...
export default function Education() {
  ...
  return (
    <>
      <script type="application/ld+json">{JSON.stringify(schemaEducation)}</script>
      <Motion.section
        ref={ref}
        id="education"
      ...
      >
        <div className="w-full max-w-5xl px-4 mx-auto">
          <h2 className="text-3xl text-white sm:text-4xl lg:text-5xl font-bold text-center mb-4" itemProp="name">Educação</h2>
              ...
              {educationData.map((edu, index) => (
                ...
                >
                  <div className="flex-shrink-0 mb-4 sm:mb-0 sm:mr-6">
                    <img src={edu.logo} alt={`Logo da instituição ${edu.institution}`} className="w-16 h-16 object-contain" itemProp="image" />
                  </div>
                  <div className="flex-1">
                    <h3 id={`edu-title-${index}`} className="text-xl sm:text-2xl font-bold text-gray-800 mb-2" itemProp="name">{edu.title}</h3>
                    <p className="text-gray-600 text-sm sm:text-base mb-4" itemProp="description">{edu.description}</p>
                    {edu.progress ? (
          ...
          {activeTab === 'certificates' && (
            <div className="overflow-y-auto max-h-[500px] space-y-4 px-6 pb-8">
              {certificates.map((cert, index) => (
                ...
                >
                  <div>
                    <h4 id={`cert-title-${index}`} className="text-lg font-semibold text-gray-800" itemProp="name">{cert.title}</h4>
                    <p className="text-sm text-gray-600">{cert.platform}</p>
                  </div>
                  <div className="text-sm text-gray-500 mt-2 sm:mt-0">
                    <a href={cert.link} target="_blank" rel="noopener noreferrer" className="text-[#012286] hover:underline" itemProp="url">
                      Ver Certificado
                    </a>
          ...
          {activeTab === 'awards' && (
            <div className="space-y-4">
              {awards.map((award, index) => (
                ...
                >
                  <h4 id={`award-title-${index}`} className="text-lg font-semibold text-gray-800" itemProp="name">{award.title}</h4>
